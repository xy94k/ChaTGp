import openai, logging, os, json, tiktoken, aiofiles
...
DEFAULT_USER_DATA = {
    ...
    'messages': [{"role": "system",
                  "content": "You are a helpful assistant. Always use <code> html tag to format program code! Do not use it in other text! Solve tasks step by step."}]
}
    ...
    try:
        ...
        if 0.0 <= temp <= 2.0:
            ...
            await message.answer(
                "Неверное значение. Используйте значения между 0.0 и 2.0. Большие значения делают ответы более случайными.")
    except IndexError:
...
async def set_max_tokens(message: types.Message):
    ...
    try:
        ...
        if 0 <= max_tokens <= 4000:
            ...
            await message.answer(
                "Неверное значение. Используйте значения до 4000. Обратите внимание, что суммарный размер запроса и ответа не должен превышать 4097 токенов.")
    except IndexError:
